...
def register_member_fcm_token(
    ...
    try:
        ...
        if not is_new_token and old_token:
            ...
            logger.debug(f"[TOKEN CHECK] 동일 토큰 감지 - 중복 방지 체크 시작: 회원 ID {request.mt_idx}")
            ...
            if member.mt_token_updated_at:
                ...
                logger.debug(f"[TOKEN CHECK] mt_token_updated_at이 None입니다")

            logger.info(f"✅ 동일 토큰 재등록 허용: 회원 ID {request.mt_idx} (2시간 이상 경과)")
            logger.debug(f"동일 토큰 재등록 허용: 회원 ID {request.mt_idx}")
...
def trigger_token_update_test(
    ...
    try:
        ...
        # 실제 register 함수 호출
        result = register_member_fcm_token(test_request, db, http_request)
