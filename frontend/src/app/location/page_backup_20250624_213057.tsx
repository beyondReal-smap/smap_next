...
export default function LocationPage() {
  ...
  const isFirstInfoWindowShownRef = useRef(false);
  const infoWindowRef = useRef<NaverInfoWindow | null>(null);

  useEffect(() => {
    isFirstInfoWindowShownRef.current = isFirstInfoWindowShown;
  ...
  useEffect(() => {
    if (isFirstInfoWindowShownRef.current || infoWindowRef.current) return;
    if (
      isMapReady &&
      map &&
      window.naver?.maps &&
