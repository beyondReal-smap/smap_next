        ...
        try {
          ...
          if (data.success) {
            ...
            if (isNewUser) {
              ...
              setTimeout(async () => {
                try {
                  console.log('[SOCIAL_LOGIN] 🔔 Google 로그인 후 FCM 토큰 체크/업데이트 시작');
                  ...
                  if (data.user?.mt_idx && fcmTokenService) {
                    ...
                    if (fcmResult.success) {
                      console.log('[SOCIAL_LOGIN] ✅ FCM 토큰 체크/업데이트 완료:', fcmResult.message);
                    } else {
                      console.warn('[SOCIAL_LOGIN] ⚠️ FCM 토큰 체크/업데이트 실패:', fcmResult.error);
                    }
                  } else {
                    console.warn('[SOCIAL_LOGIN] ⚠️ FCM 토큰 체크/업데이트 스킵: mt_idx 없음 또는 fcmTokenService 초기화 실패');
                  }
                } catch (fcmError) {
                  console.error('[SOCIAL_LOGIN] ❌ FCM 토큰 체크/업데이트 중 오류:', fcmError);
                }
