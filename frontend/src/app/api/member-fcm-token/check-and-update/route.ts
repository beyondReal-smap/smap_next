...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Member FCM Token API] 백엔드 요청:', backendUrl);
    console.log('[Member FCM Token API] 요청 데이터:', { mt_idx, token_length: fcm_token.length });
    ...
    console.error('[Member FCM Token API] 오류:', error);
    return NextResponse.json(
      { 
        success: false,
