...
class ActivityLogService {
    ...
    func getDailyLocationCounts(groupId: Int, days: Int = 14) async throws -> DailyCountsResponse {
        guard let url = URL(string: "\(baseURL)/api/v1/member-location-logs/daily-counts?group_id=\(groupId)&days=\(days)") else {
    ...
    func getMapMarkers(memberId: Int, date: String, minSpeed: Double = 1.0, maxAccuracy: Double = 50.0) async throws -> [MapMarker] {
        guard let url = URL(string: "\(baseURL)/api/v1/member-location-logs/\(memberId)/map-markers?date=\(date)&min_speed=\(minSpeed)&max_accuracy=\(maxAccuracy)") else {
            throw URLError(.badURL)
        }
    ...
    func getStayTimes(memberId: Int, date: String, minSpeed: Double = 1.0, maxAccuracy: Double = 50.0, minDuration: Int = 5) async throws -> [StayTime] {
        guard let url = URL(string: "\(baseURL)/api/v1/member-location-logs/\(memberId)/stay-times?date=\(date)&min_speed=\(minSpeed)&max_accuracy=\(maxAccuracy)&min_duration=\(minDuration)") else {
            throw URLError(.badURL)
        }
    ...
    func getLocationLogSummary(memberId: Int, date: String) async throws -> LocationSummary? {
        guard let url = URL(string: "\(baseURL)/api/v1/member-location-logs") else {
            throw URLError(.badURL)
        }
    ...
    func getMemberActivityByDate(groupId: Int, date: String) async throws -> MemberActivityResponse? {
        ...
        guard let url = URL(string: "\(baseURL)/api/v1/member-location-logs/member-activity?group_id=\(groupId)&date=\(date)&_t=\(timestamp)") else {
            throw URLError(.badURL)
        }
    ...
    func getMapMarkersPublisher(memberId: Int, date: String) -> AnyPublisher<[MapMarker], Error> {
        guard let url = URL(string: "\(baseURL)/api/v1/member-location-logs/\(memberId)/map-markers?date=\(date)&min_speed=1.0&max_accuracy=50.0") else {
            return Fail(error: URLError(.badURL)).eraseToAnyPublisher()
        }
